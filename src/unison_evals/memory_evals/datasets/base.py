...
class Dataset(ABC):
    ...
    supported_tracks: frozenset[Track] = frozenset(
        {Track.AGENT_ORACLE}
    )  # every dataset can serve Track 2 via load(); brain/scale opt in.
    ...
    def load_brain_questions(self, limit: int | None = None) -> Iterable[BrainQuestion]:
        """Track 1 / Track 3 variant — yields BrainQuestion (with corpus + gold doc paths).

        Default implementation raises NotImplementedError so the runner can
        report a clear 'this dataset doesn't support Track 1' message.
        Override per-dataset.
        """
        raise NotImplementedError(
            f"{self.name} does not yet support Track 1 / Track 3 (needs a BrainQuestion converter)."
        )
