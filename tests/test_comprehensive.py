...
def test_comprehensive_fallback_file_scan(tmp_path: Path):
            ...
            "summaries": [
                {
                    ...
                    "pass_rate": 0.80,
                    "total_cost_usd": 0.001,
                    "cost_per_question_usd": 0.0002,
                    "cost_per_solved_usd": 0.00025,
                    "avg_latency_ms": 50.0,
                    "p50_latency_ms": 45.0,
                    "p95_latency_ms": 90.0,
                }
    ...
    (comp_dir / "agent-longmemeval-unison-agent.json").write_text(json.dumps(combo_data))
    ...
    combos = group["combos"]
    assert len(combos) == 1
    assert combos[0]["system"] == "unison-agent"
    assert combos[0]["pass_rate"] == pytest.approx(0.80)
