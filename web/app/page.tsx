...
export default async function HomePage() {
  let runs: RunDetail[] = [];
  let comprehensiveGroups: ComprehensiveGroup[] = [];
  let loadError: string | null = null;
  try {
    runs = await listRuns();
    const compResp = await listComprehensiveRuns();
    comprehensiveGroups = compResp.runs;
  } catch (e) {
    ...
    <div className="space-y-6">
      ...
      {!loadError && (
        <Suspense fallback={<p className="text-sm text-[color:var(--muted)]">Loading…</p>}>
          <HomeClient runs={runs} comprehensiveGroups={comprehensiveGroups} />
        </Suspense>
      )}
    </div>
  );
