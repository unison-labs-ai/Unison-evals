...
// ---------------------------------------------------------------------------
// Comprehensive runs (results/comprehensive-<TS>/ on disk)
// ---------------------------------------------------------------------------
...
export async function listComprehensiveRuns(): Promise<{ runs: ComprehensiveGroup[] }> {
  const res = await fetch(apiUrl("/api/comprehensive"));
  if (!res.ok) throw new Error(`List comprehensive runs failed: ${res.status}`);
  return res.json();
}
